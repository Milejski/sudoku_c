...
char start_up();
char print_menu(char *header, char *menus[], char number);
void new_game();
void load_game();
void options_menu();
...
static unsigned char selectx=0;
...
int main(){
	...
	char main_header[]="SUDOKU";
	char *main_menu[5]={"New game", "Load game", "Add new sudoku", "Options", "Exit"};
	while(run_main_menu){
		switch(print_menu(main_header,main_menu,5)){
			case 0:
				reset_position();
				new_game();
				break;
...
char print_menu(char* header, char *menus[], char number){
	...
	pressed=getch();
	if(pressed==config[2]){
		if(selecty==0){selecty=number-1;}
		else{selecty--;}
	}
	if(pressed==config[3]){
		if(selecty==number-1){selecty=0;}
		else{selecty++;}
	}
	if(pressed==config[4]){
		return selecty;
...
void new_game(){
	char new_game_header[]="NEW SUDOKU";
	char *new_game_options[3]={"Chose random sudoku","Chose sudoku by number","Return to main menu"};
	char run_new_game_options=1;
	FILE *f=fopen(file_name,"rb+");
	...
	fclose(f);
	if(numb==0){
		printf("Add sudoku to data first!\n");
		getch();
		run_new_game_options=0;
	}
	while(run_new_game_options){
		switch(print_menu(new_game_header,new_game_options,3)){
			case 0:
				reset_position();
				srand(time(NULL));
			...
			case 1:
				reset_position();
				system("cls");
				printf("Number of sudokus: %d\nEnter sudoku number, from 1 to %d: ",numb,numb);
				int c=0;
				scanf("%d",&c);
				c--;
				if(c>=0&&c<numb){
...
void load_game(){
	char i,j,c;
	char loaded_sudoku_header[]="LOADED SUDOKU";
	char *loaded_sudoku_menu[4]={"Check sudoku","Return to sudoku","Save sudoku and quit","Quit sudoku without saving"};
	char sudoku[9][9]={{0}};
	char solved[9][9]={{0}};
	FILE *f=fopen(file_name,"rb+");
	...
	unsigned char run_loaded_sudoku=1;
	unsigned char run_loaded_menu;
	while(run_loaded_sudoku){
		switch(print_sudoku(loaded_sudoku_header,sudoku)){
			case 0:
				reset_position();
				run_loaded_menu=1;
				while(run_loaded_menu){
					switch(print_menu(loaded_sudoku_header,loaded_sudoku_menu,4)){
						case 0:
							reset_position();
							c=0;
							for(i=0;i<9;i++){
...
void options_menu(){
	...
	char options_header[]="OPTIONS MENU";
	char *options_menu[5]={"Change control scheme", "Change background and text color", "Return to main menu"};
	char run_control_menu;
	char control_header[]="CHANGE CONTROL SCHEME";
	char *control_menu[]={"Left","Right","Up","Down","Select","Return to options menu"};
	while(run_options_menu){
		switch(print_menu(options_header,options_menu,3)){
			case 0:
				reset_position();
				run_control_menu=1;
				while(run_control_menu){
					system("cls");
					printf("\t%s\n\n",control_header);
					if(selecty==0){printf(" -->");}
					printf("\t%s\t= %c\n",control_menu[0],config[0]);
					if(selecty==1){printf(" -->");}
					printf("\t%s\t= %c\n",control_menu[1],config[1]);
					if(selecty==2){printf(" -->");}
					printf("\t%s\t= %c\n",control_menu[2],config[2]);
					if(selecty==3){printf(" -->");}
					printf("\t%s\t= %c\n",control_menu[3],config[3]);
					if(selecty==4){printf(" -->");}
					printf("\t%s\t= ",control_menu[4]);
...
void play_sudoku(int number){
	...
	char *sudoku_options_menu[4]={"Check sudoku","Return to sudoku","Save sudoku and quit","Quit sudoku without saving"};
	char sudoku[9][9];
	char solved[9][9];
	FILE *f=fopen(file_name,"rb+");
	...
	while(run_play_sudoku){
		switch(print_sudoku(play_sudoku_header,sudoku)){
			case 0:
				...
				run_play_menu=1;
				while(run_play_menu){
					switch(print_menu(play_sudoku_header,sudoku_options_menu,4)){
						case 0:
							reset_position();
							c=0;
							for(i=0;i<9;i++){
...
void addnew_menu(){
	char i,j,c=0;
	char addnew_header[]="ADDING NEW SUDOKU";
	char addnew_header2[]="NEW SUDOKU OPTIONS";
	char *addnew_options[5]={"Solve sudoku","Add sudoku to data","Return to sudoku","Erase sudoku","Return to main menu"};
	...
	//char sudoku[9][9]={{0}};
	char solved[9][9]={{0}};
	unsigned char run_addnew_sudoku=1;
	unsigned char run_addnew_options;
	while(run_addnew_sudoku){
		switch(print_sudoku(addnew_header,sudoku)){
			case 0:
				reset_position();
				run_addnew_options=1;
				while(run_addnew_options){
					switch(print_menu(addnew_header2,addnew_options,5)){
						case 0:
							reset_position();
							for(i=0;i<9;i++){
								for(j=0;j<9;j++){
						...
						case 1:
							...
							if(!c){
								system("cls");
								printf("To add sudoku, chose solve option first.\n");
							}else{
								FILE *f=fopen(file_name,"rb+");
								int numb;
								fseek(f,sizeof(char)*7,SEEK_SET);
								...
								fwrite(&solved[0][0],sizeof(char),81,f);
								fclose(f);
								printf("Sudoku has been added to database.\n");
							}										
							getch();
							break;
						case 2:
